...
Submission artifacts prepared in this repo:

- [`submission/RUN_LOG.md`](submission/RUN_LOG.md): captured output for `make verify`,
  `make test`, `make flywheel`, and `dbt build`.
- [`submission/REFLECTION.md`](submission/REFLECTION.md): short reflection for the
  submission prompt.
- [`datasets/eval_golden.jsonl`](datasets/eval_golden.jsonl) and
  [`datasets/preference_pairs.jsonl`](datasets/preference_pairs.jsonl): artifacts
  generated by `make flywheel`.
- [`bonus/DESIGN.md`](bonus/DESIGN.md): optional brainstorm write-up for the +20 bonus.
